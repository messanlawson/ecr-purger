...
# for instance if an image (i.e builds/<image-name>) is used accross various services then ecr-purger will inspect # the respective old taskdefs (latest taskdef - wiggle-room/n revisions) for these services and collect the imagePushedAt time for each version of this container used accross these old taskdef # revisions and use the oldest imagePushedAt time found to determine what
